...
function nowPlayingUiSongMatches(song, opts) {
  opts = opts || {};
  if (!song) return false;
  if (opts.token != null && Number(opts.token) !== Number(trackSwitchToken)) return false;
  if (opts.force) return true;
  var active = typeof currentCoverSong === 'function' ? currentCoverSong() : null;
  if (!active) return true;
  if (song === active) return true;
  var songToken = Number(song.__shinayuuTrackToken || 0);
  var activeToken = Number(active.__shinayuuTrackToken || 0);
  if (songToken && activeToken) return songToken === activeToken && songToken === Number(trackSwitchToken);
  if (typeof queueItemKey === 'function') return queueItemKey(song) === queueItemKey(active);
  return String(song.id || song.mid || '') === String(active.id || active.mid || '');
}
function updateControlTrackInfo(song, opts) {
  song = song || {};
  opts = opts || {};
  if (!nowPlayingUiSongMatches(song, opts)) return false;
  ...
  if (title) {
    ...
    var visibleTitle = song.name || song.title || '';
    if (titleText) titleText.textContent = visibleTitle;
    else title.textContent = visibleTitle;
    if (titleBadges) {
  ...
  if (artist) artist.textContent = song.artist || song.singer || '';
