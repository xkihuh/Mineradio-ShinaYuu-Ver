function normalizeShinaYuuLocalMediaUrl(value) {
  var src = String(value || '').trim();
  // The native service exposes only files inside the folder selected by the
  // user, encoded as a base64url token. Never accept arbitrary file:// paths.
  return /^shinayuu-media:\/\/local\/[A-Za-z0-9_-]+$/i.test(src) ? src : '';
}
function normalizeCustomBackgroundImage(value) {
  var src = String(value || '').trim();
  if (!src) return '';
  if (normalizeShinaYuuLocalMediaUrl(src)) return src;
...
function normalizeCustomBackgroundMedia(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (normalizeShinaYuuLocalMediaUrl(value) || /^data:video\/(mp4|webm|quicktime);base64,/i.test(value) || /^https?:\/\//i.test(value)) return { type: 'video', src: String(value) };
    return null;
  }
  ...
  if (type === 'image') {
    ...
    return imageSrc ? {
      ...
      folderPath: String(value.folderPath || '').slice(0, 1000),
      relativePath: String(value.relativePath || '').slice(0, 1000)
    } : null;
  }
  if (type === 'video') {
    var src = String(value.src || '').trim();
    var id = String(value.id || '').trim();
    if (!id && !normalizeShinaYuuLocalMediaUrl(src) && !/^data:video\/(mp4|webm|quicktime);base64,/i.test(src) && !/^https?:\/\//i.test(src)) return null;
    return {
      type: 'video',
      id: id,
      src: src,
