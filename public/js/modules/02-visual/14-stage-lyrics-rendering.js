...
function stageLyricLivePlaybackSeconds() {
  // Spotify Premium is rendered by the Web Playback SDK. Its clock lives in
  // getPlaybackCurrentSeconds(), while the HTMLAudioElement is intentionally
  // emptied. Reading audio.currentTime here therefore pins Spotify lyrics at
  // zero and makes a seek preview show the correct line only until pointerup.
  if (typeof getPlaybackCurrentSeconds === 'function') {
    try {
      var providerSeconds = Number(getPlaybackCurrentSeconds());
      if (isFinite(providerSeconds) && providerSeconds >= 0) return providerSeconds;
    } catch (_) {}
  }
...
function stageLyricLinesContainRealLyrics() {
  return (Array.isArray(lyricsLines) ? lyricsLines : []).some(function (line) {
    if (!line || line.fallback) return false;
    var value = String(line.text || '').trim();
    if (!value) return false;
    return typeof isNoLyricText !== 'function' || !isNoLyricText(value);
  });
}
...
function stageLyricRestoreWarmupSeconds() {
  ...
  var actual = stageLyricLivePlaybackSeconds();
...
function stageLyricPlaybackSeconds() {
  ...
  return stageLyricLivePlaybackSeconds();
}
...
function updateStageLyrics3D(dt) {
  ...
  var shelfDetailBehind = !!(shelfDetailOpen && typeof shelfLyricsBehind !== 'undefined' && shelfLyricsBehind);
  ...
  // The original lyric material does not use depth testing, so an explicit
  // render order restores the intended UI contract reliably: lyrics are above
  // the shelf by default, and only move behind after the user right-clicks.
  var stageLyricRenderBase = shelfDetailBehind ? 24 : 420;
  stageLyrics.group.renderOrder = stageLyricRenderBase;
  var shelfDetailLyricProfile = shelfDetailBehind ? {
  ...
  function tickMesh(mesh, isCurrent) {
    ...
    if (isCurrent) {
      ...
      var opacityEase = shelfDetailBehind && currentOpacity > lyricOpacityTarget ? shelfDetailLyricProfile.easeDown : 0.16;
      ...
      if (data.readabilityMat) {
        ...
        var readabilityEase = shelfDetailBehind && data.readabilityMat.opacity > readabilityTarget ? 0.28 : 0.16;
      ...
      if (data.textMat && data.textMat.uniforms.uSolar) {
        ...
        var solarEase = shelfDetailBehind && data.textMat.uniforms.uSolar.value > solarTarget ? 0.26 : 0.12;
      ...
      if (data.glowMat) {
        ...
        data.glowMat.opacity += (glowTarget - data.glowMat.opacity) * (glowTarget > data.glowMat.opacity ? 0.095 : (shelfDetailBehind ? 0.20 : 0.055));
        data.glowMat.color.copy(lyricStageGlowThreeColor(stageLyrics.palette, '#9cffdf', 0.36)).lerp(lyricSunHotColor, warmth);
      }
      if (data.sparkMat) {
        var sparkTarget = lyricGlowStrength > 0 && fx.lyricGlowParticles && !shelfDetailBehind ? Math.min(0.42, (0.10 + solar * 0.14 + stageLyrics.beatGlow * 0.10) * Math.min(1.6, glowDrive)) : 0;
        var sparkOpacity = getLyricSparkOpacity(data);
        sparkOpacity += (sparkTarget - sparkOpacity) * (sparkTarget > sparkOpacity ? 0.13 : (shelfDetailBehind ? 0.22 : 0.075));
        setLyricSparkOpacity(data, sparkOpacity);
        var sparkSizeTarget = fx.lyricGlowParticles && !shelfDetailBehind ? (0.050 + solar * 0.016 + stageLyrics.beatGlow * 0.026 + bass * 0.008) : 0.035;
      ...
      if (data.sunMat) {
        var sunTarget = lyricGlowStrength > 0 && !shelfDetailBehind ? Math.min(0.88, (Math.pow(Math.min(1.35, solar), 1.08) * 0.28 + stageLyrics.beatGlow * 0.20) * Math.min(2.4, glowDrive) * (style === 'shine' ? 1.18 : 1)) : 0;
        if (data.suppressStaticGlow) sunTarget = 0;
        data.sunMat.opacity += (sunTarget - data.sunMat.opacity) * (shelfDetailBehind ? 0.18 : 0.055);
    ...
    if (data.textMat) data.textMat.uniforms.uOpacity.value = opacity;
    if (data.readabilityMat) data.readabilityMat.opacity = opacity * (shelfDetailBehind ? shelfDetailLyricProfile.readability : 0.58);
    if (data.contextMat) data.contextMat.opacity = opacity * (shelfDetailBehind ? shelfDetailLyricProfile.readability * 0.72 : clampRange(0.46 + lyricContextOpacityValue() * 0.26, 0.54, 0.76));
    updateLyricRowLayers(data, {
      opacity: opacity,
      readability: shelfDetailBehind ? shelfDetailLyricProfile.readability : 0.58,
      contextIntro: 1,
      shownProgress: shownProgress,
    ...
    if (data.textMat && data.textMat.uniforms.uSolar) data.textMat.uniforms.uSolar.value *= shelfDetailBehind ? 0.72 : 0.86;
    if (data.glowMat) data.glowMat.opacity = (!data.suppressStaticGlow && lyricGlowStrength > 0) ? (shelfDetailBehind ? Math.min(shelfDetailLyricProfile.glowCap * 0.40, opacity * 0.05 * lyricGlowStrength) : opacity * 0.08 * lyricGlowStrength) : 0;
    if (data.sparkMat) {
      var outgoingSpark = lyricGlowStrength > 0 && fx.lyricGlowParticles && !shelfDetailBehind ? Math.max(opacity * 0.24 * lyricGlowStrength, (1 - a) * 0.18 * lyricGlowStrength) : 0;
      setLyricSparkOpacity(data, outgoingSpark);
      setLyricSparkSize(data, 0.046 + (1 - a) * 0.020);
    }
    if (data.sunMat) data.sunMat.opacity = (!data.suppressStaticGlow && lyricGlowStrength > 0 && !shelfDetailBehind) ? opacity * 0.08 * lyricGlowStrength : 0;
    if (previewMotionLock) {
      mesh.position.set(
        Number(mesh.userData.progressPreviewHoldX) || 0,
        Number(mesh.userData.progressPreviewHoldY) || 0,
...
function tickLyricsParticles() {
  ...
  if (newIdx < 0) {
    // A track that already has real timed lyrics must never be replaced by the
    // song title merely because its first lyric starts after an instrumental
    // intro. Keep the title fallback exclusively for genuinely lyric-less
    // tracks. This also prevents a stale title mesh from covering Spotify
    // lyrics after the SDK clock is reconciled following a seek.
    if (stageLyricLinesContainRealLyrics()) {
      if (stageLyrics.currentIdx === -2 || (stageLyrics.current && stageLyrics.current.userData && stageLyrics.current.userData.lyric && stageLyrics.current.userData.lyric.fallback)) {
        retireCurrentStageLyricForIdle();
      }
