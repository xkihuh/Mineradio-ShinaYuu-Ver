...
function activateFocusZone(type) {
  ...
  if (type === 'shelf-side') {
    ...
    // Keep the lyric stage readable while the left HTML shelf is hovered. The
    // previous radius pushed the current lyric too close to the camera.
    orbit.focus.theta = 0.32;
    orbit.focus.phi = 0.045;
    orbit.focus.radius = 6.42;
    orbit.focus.lookAt.set(-0.92, 0.01, 0.08);
  }
}
