(function () {
  ...
  function applyControlPanelOpacity(value, persist, custom) {
    value = clamp(value, 0.90, 0.05, 0.96);
    var sticky = clamp(value * 0.88, 0.07, 0.05, 0.86);
    var card = clamp(value * 0.68, 0.06, 0.04, 0.78);
  ...
  function applyRightShelfBackgroundOpacity(value, persist) {
    value = clamp(value, 0.79, 0.05, 0.98);
  ...
  function bindLiquidGlass() {
    var panel = readStored(CONTROL_PANEL_KEY, 0.90, 0.05, 0.96);
    var left = readStored(LEFT_SHELF_KEY, 0.90, 0.05, 0.96);
    var fxRight = window.fx && Number(window.fx.shelfOpacity);
    var rightDefault = Number.isFinite(fxRight) ? clamp(fxRight, 1, 0.05, 1) : 1;
    var right = readStored(RIGHT_SHELF_KEY, rightDefault, 0.05, 1);
    var fxRightBg = window.fx && Number(window.fx.shelfBgOpacity);
    var rightBgDefault = Number.isFinite(fxRightBg) ? clamp(fxRightBg, 0.79, 0.05, 0.98) : 0.79;
    var rightBg = readStored(RIGHT_SHELF_BG_KEY, rightBgDefault, 0.05, 0.98);
    ...
    if (original && original.dataset.syLiquidGlassMirror !== '1') {
      ...
      var mirror = function () {
        var value = clamp(original.value, 1, 0.05, 1);
        writeStored(RIGHT_SHELF_KEY, value);
    ...
    if (originalBg && originalBg.dataset.syLiquidGlassMirror !== '1') {
      ...
      var mirrorBg = function () {
        var value = clamp(originalBg.value, 0.79, 0.05, 0.98);
        writeStored(RIGHT_SHELF_BG_KEY, value);
