...
test('2.1.5 cancels stale AutoMix fades before a new root playback selection', () => {
...
test('2.1.5 restores all output paths and releases stale execution locks', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
...
test('2.1.5 never unloads a prepared deck after it becomes the primary media element', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
