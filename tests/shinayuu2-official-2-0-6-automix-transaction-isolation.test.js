...
test('2.0.14 cancels stale AutoMix fades before a new root playback selection', () => {
...
test('2.0.14 restores all output paths and releases stale execution locks', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
...
test('2.0.14 never unloads a prepared deck after it becomes the primary media element', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
...
test('release identity is consistently bumped to 2.0.14', () => {
  ...
  assert.equal(lock.version, '2.0.14');
  assert.equal(lock.packages[''].version, '2.0.14');
});
