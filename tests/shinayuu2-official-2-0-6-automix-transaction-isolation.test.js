...
test('2.0.6 cancels stale AutoMix fades before a new root playback selection', () => {
...
test('2.0.6 restores all output paths and releases stale execution locks', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
  assert.match(mix, /function restoreAutoMixOutput\(reason\)/);
  assert.match(mix, /window\.audio\.muted = false/);
  assert.match(mix, /window\.audio\.playbackRate = 1/);
  assert.match(mix, /Promise\.resolve\(setSpotifyVolume\(savedVolume\)\)/);
...
test('2.0.6 never unloads a prepared deck after it becomes the primary media element', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
...
test('release identity is consistently bumped to 2.0.6', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  assert.equal(pkg.version, '2.0.6');
  assert.equal(pkg.displayVersion, '2.0.6');
  assert.equal(pkg.shinayuu.displayVersion, '2.0.6');
  assert.equal(pkg.build.buildVersion, '2.0.6.0');
  assert.equal(lock.version, '2.0.6');
  assert.equal(lock.packages[''].version, '2.0.6');
});
