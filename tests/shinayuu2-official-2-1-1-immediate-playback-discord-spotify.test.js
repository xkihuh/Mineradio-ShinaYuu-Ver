...
test('2.1.1 keeps the Discord Connect card Liquid Glass without relying on one external CSS response', () => {
  const html = read('public/index.html');
  assert.match(html, /id="shinayuu-discord-critical-2-1-1"/);
  assert.match(html, /shinayuu-alpha3\.0\.5-fixes\.css\?v=2\.1\.1/);
...
test('2.1.1 manual playback has no former 2.8-second AutoMix release gate', () => {
...
test('2.1.1 Spotify startup waits only for a concrete old-provider stop and prewarms non-blockingly', () => {
...
test('2.1.1 package and bundled assets use the synchronized release identity', () => {
  ...
  assert.match(html, /spotify-direct-player\.js\?v=2\.1\.1/);
  assert.match(html, /shinayuu-index-bundle\.js\?v=2\.1\.1/);
});
