...
test('official identity is ShinaYuu Music 2.0.8', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  assert.equal(pkg.version, '2.0.8');
  assert.equal(pkg.displayVersion, '2.0.8');
  assert.equal(pkg.shinayuu.displayVersion, '2.0.8');
  assert.equal(pkg.build.buildVersion, '2.0.8.0');
  assert.equal(lock.version, '2.0.8');
});
