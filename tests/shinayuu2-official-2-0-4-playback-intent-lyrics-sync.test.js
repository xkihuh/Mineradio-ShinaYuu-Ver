...
test('2.0.15 isolates every root playback selection from stale provider recovery', () => {
...
test('Liquid lyrics timing UI keeps ±15s global/per-track correction and removes only the title wait', () => {
  ...
  assert.doesNotMatch(html, /lyric-title-wait-slider|Thời gian chờ lyrics trước khi hiện tên bài|Chờ lyrics trước khi hiện tên bài/);
  ...
  assert.doesNotMatch(timing, /getLyricTitleFallbackDelayMs|setLyricTitleFallbackWaitSeconds|LYRIC_TITLE_FALLBACK_WAIT/);
  assert.match(lyrics, /var fallbackDelay = multiLineDelay/);
...
test('lyrics clock combines provider time, global delay and per-track correction deterministically', () => {
  ...
  assert.equal(context.getAdjustedLyricPlaybackTime(10), 0);
});

test('release identity is consistently bumped to 2.0.15', () => {
  const pkg = JSON.parse(read('package.json'));
  assert.equal(pkg.version, '2.0.15');
  assert.equal(pkg.displayVersion, '2.0.15');
  assert.equal(pkg.shinayuu.displayVersion, '2.0.15');
  assert.equal(pkg.build.buildVersion, '2.0.15.0');
});
