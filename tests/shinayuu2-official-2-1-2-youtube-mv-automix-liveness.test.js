...
test('2.1.4 starts exact selected-MV captions beside QQ and NetEase and gives exact timing priority', () => {
...
test('2.1.4 never animates evenly spaced lyrics while exact MV alignment is pending', () => {
  const providers = read('music-providers.js');
...
test('2.1.4 does not duration-stretch exact caption or completed alignment timestamps', () => {
  const parser = read('public/js/modules/06-lyrics/00-lyrics-fetch-parse.js');
...
test('2.1.4 retains unresolved provider-stop barriers and protects a newer Spotify request', () => {
...
test('2.1.4 AutoMix has a bounded transition transaction and preserves the outgoing provider on timeout', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
  assert.match(mix, /var VERSION = '2\.1\.4'/);
