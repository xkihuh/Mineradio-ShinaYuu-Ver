...
test('2.1.3 serializes Spotify AutoMix volume commands instead of leaving overlapping writes', () => {
...
test('2.1.3 restores only the provider that owns audible output after AutoMix', () => {
  const mix = read('public/js/modules/05-playback/18-cuefield-automix-integration.js');
...
test('a late Spotify stop cannot clear a newer HTML transport or its play state', () => {
  ...
  assert.match(spotify, /if \(stopStillOwnsState && !htmlAlreadyOwnsOutput\)/);
